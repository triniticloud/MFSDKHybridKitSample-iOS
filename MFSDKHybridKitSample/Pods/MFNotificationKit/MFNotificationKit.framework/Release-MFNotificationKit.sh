...
#device builds
xcodebuild -scheme MFNotificationKit -workspace WebSDK.xcworkspace -sdk iphoneos -configuration Release build BUILD_DIR="${COMPILER_OUTPUT}/build" DEVELOPMENT_TEAM=S8ALLG76VY

echoTask "DEVICE BUILD DONE"

#simulator builds
xcodebuild -scheme MFNotificationKit -workspace WebSDK.xcworkspace -sdk iphonesimulator -configuration Release build BUILD_DIR="${COMPILER_OUTPUT}/build" DEVELOPMENT_TEAM=S8ALLG76VY
...
for PROJECT_NM in "MFNotificationKit"
do
...
zip -r MFNotificationKit.zip MFNotificationKit.framework

echoTask "MFNotificationKit.zip CREATED"

cd "${COMPILER_OUTPUT}"

curl -umobile:AP4rEqYoNAZEY9YNKZWVTATEScuQ7QCvcBEbxp -T "${COMPILER_OUTPUT}/build/Release-universal/MFNotificationKit.zip" "${MF_COMPLETEURL}"
...
pod spec lint MFNotificationKit.podspec --allow-warnings
echoTask "POD VALIDATION DONE"

pod trunk push MFNotificationKit.podspec --allow-warnings
